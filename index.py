...
class InvertedIndex:
    ...
    postings_dict: Dictionary mapping:
        ...
        yang memetakan term ID (integer) ke 5-tuple:
           ...
           5. max_tf : nilai TF terbesar di postings list ini. Digunakan untuk
              menghitung upper bound score pada WAND retrieval.
...
class InvertedIndexReader(InvertedIndex):
    ...
    def __next__(self):
        ...
        entry = self.postings_dict[curr_term]
        # Support both 4-tuple (legacy) and 5-tuple (with max_tf)
        pos = entry[0]
        len_in_bytes_of_postings = entry[2]
        len_in_bytes_of_tf = entry[3]
    ...
    def append(self, term, postings_list, tf_list):
        ...
        3. Menyimpan metadata dalam bentuk self.terms, self.postings_dict, dan self.doc_length.
           ...
           sebuah 5-tuple: - start_position_in_index_file
                           ...
                           - length_in_bytes_of_postings_list
                           - length_in_bytes_of_tf_list
                           - max_tf (nilai TF terbesar untuk WAND upper bound)
        ...
        5. Update self.terms dan self.doc_length.
        ...
        # Simpan max_tf untuk WAND upper bound computation
...
if __name__ == "__main__":
    ...
    for PostingsClass in [VBEPostings, EliasGammaPostings]:
        print(f"=== Testing with {PostingsClass.__name__} ===")
        with InvertedIndexWriter('test', postings_encoding=PostingsClass, directory='./tmp/') as index:
            ...
            # Verifikasi postings_dict format (5-tuple dengan max_tf)
            entry1 = index.postings_dict[1]
            assert entry1[0] == 0, "start position salah"
            assert entry1[1] == 5, "number of postings salah"
            assert entry1[4] == 30, f"max_tf salah: expected 30, got {entry1[4]}"

            entry2 = index.postings_dict[2]
            assert entry2[4] == 56, f"max_tf salah: expected 56, got {entry2[4]}"

            index.index_file.seek(index.postings_dict[2][0])
            assert PostingsClass.decode(index.index_file.read(entry2[2])) == [3,4,5], "postings decode salah"
            assert PostingsClass.decode_tf(index.index_file.read(entry2[3])) == [34,23,56], "tf decode salah"

        print(f"{PostingsClass.__name__}: semua test PASSED!")

    print("\nSemua test index.py PASSED!")
